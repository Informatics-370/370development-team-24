...
//         this.snackbar.open('Drink type deleted successfully.', '', { duration: 5000 });
//         window.location.reload();
//       },
//       (error) => {
//         // Deletion failed
//         this.deletionSuccess = false;
//         this.snackbar.open('Cannot delete drink type because it is linked to drinks.', '', { duration: 5000 });
//       }
//     );
//   }
...
export class DrinkTypeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.GetAllDrinkTypes();
    this.filteredDrinkTypes = this.drinkType; // Initialize the filtered list
  }

  GetAllDrinkTypes() {
    this.dataService.GetAllDrinkTypes().subscribe((result) => {
      this.drinkType = result as DrinkType[];
    });
  }
